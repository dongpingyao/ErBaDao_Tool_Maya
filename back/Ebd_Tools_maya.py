...
for i in sys.path:
    if i.split('/')[-1] == 'ErBaDao_Tool_Maya' or i.split('\\')[-1] == 'ErBaDao_Tool_Maya':
        ...
        UIFile = i + "/ui/Ebd_Tools_maya_betaV214.ui"
        pass
if ErBaDao_Tool_Maya == 'True':
    ...
    for i in sys.path:
        if os.path.exists(i + r"/ErBaDao_Tool_Maya"):
            ...
            UIFile = i + r"/ErBaDao_Tool_Maya/ui/Ebd_Tools_maya_betaV214.ui"
        else:
            pass

import cv2CvtFormat as cv2CvtFormat
...
class EbdToolsMaya(MayaQWidgetBaseMixin, QtWidgets.QMainWindow):
    ...
    def create_connections(self):
        ...
        self.widget.cancelButton.clicked.connect(self.close)
        ...
        # 复制替换_根据点序号 逐点拓扑
            ...
            for c in sele:
                if c == sele[-1]:
                    ...
                    bb = cmds.polyEvaluate(sele[-1], v=1)
                    if aa != bb:
                        print ('与目标多边形点数不同,请手动对齐对象或者使用坐标轴')
                        zz.append(c)
                        pass
                    else:
                        # 复制源物体
                        ...
                        self.widget.Replace_progressBar.setValue(a / (len(sele) - 1) * 100)
            cmds.select(zz)
            cmds.warning('与目标多边形点数不同,请手动对齐对象或者使用坐标轴，使用实例替换试试')
        else:
            sele = cmds.ls(sl=1)
            a = 0
            for i in sele:
    ...
    def selectExistingFormat(self):
        ...
        for c in sele:
            ...
            if Path.split('\\')[-1].split('.')[-1] == Format:
                newSele.append(c)
                pass
            else:
                pass
        if len(newSele) == 0:
            cmds.warning('没有找到要转换的file节点')
        else:
            cmds.select(newSele)
    ...
    def txCvtFormat(self):
        ...
        if targetFormat == existingFormat:
            cmds.warning('请设置要转换的格式 并重试', noContext=1)
            pass
        else:
            pass
        ...
        normalFilePathList = []
        seleFilePathList = []
        print('获取选择file节点的绝对路径并修改新的绝对路径')
        ...
        print('file节点路径更新完成，进行分类UDIM贴图与普通贴图')
        ...
        for i in (udimList + normalFilePathList):
            ...
            os.remove(i)
        print('贴图格式转换程序已经运行完毕,请进行检查')
        print('备份文件夹地址'+backupFilePath)
    ...
    def txRename(self):
        ...
        if RsNormal_Node is None:
            ...
            cmds.warning('检测到材质球下有RsNormalMap节点，请转换为RsBumpMap节点后重试')
        #
    ...
    def procedureCVTtexRun(self):
        ...
        if ErDTxtName in cmds.ls(type='place2dTexture'):
            cmds.error('场景内已经有此名称的节点，换个名字试一试')
        ...
        for i in ShaderInput:
            ...
            if i.split('.')[-1] == 'diffuse_color':
                ...
                cmds.warning('程序只支持 BaseColor、Roughness、Metallic、Opacity、Emissive')
                pass
