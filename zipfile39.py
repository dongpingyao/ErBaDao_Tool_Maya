...
try:
    import zstandard # We may need its compression method
except ImportError:
    zstandard = None
        ...
        while len(extra) >= 4:
            ...
            if tp == 0x0001:
                ...
                try:
                    ...
                    raise BadZipFile("Corrupt zip64 extra field. %s not found."%field)
...
def _check_compression(compression):
    ...
    elif compression == ZIP_LZMA or compression == ZIP_XZ:
        if not lzma:
            raise RuntimeError(
...
class ZipExtFile(io.BufferedIOBase):
    ...
    def _read1(self, n):
        ...
        if self._compress_type == ZIP_STORED:
            ...
            self._eof = (getattr(self._decompressor, 'eof', False) or
...
class _ZipWriteFile(io.BufferedIOBase):
    ...
    def write(self, data):
        ...
        self._crc = crc32(data, self._crc) & 0xffffffff
        if self._compressor:
    ...
    def close(self):
        if self.closed:
            return
        try:
            super(_ZipWriteFile, self).close()
...
class PyZipFile(ZipFile):
    ...
    def _get_codename(self, pathname, basename):
        ...
        if importlib_util is not None:
            pycache_opt0 = importlib_util.cache_from_source(file_py, optimization='')
            pycache_opt1 = importlib_util.cache_from_source(file_py, optimization=1)
            pycache_opt2 = importlib_util.cache_from_source(file_py, optimization=2)
        else:
            pycache_opt0 = file_pyc
            pycache_opt1 = file_pyo
            pycache_opt2 = file_pyo
        if self._optimize == -1:
    ...
    def open(self, mode='r', *args, **kwargs):
        ...
        if self.is_dir():
            if sys.version_info[0]>=3:
                ...
                raise OSError(errno.EISDIR, self)
        zip_mode = mode[0]
        if not self.exists() and zip_mode == 'r':
            if sys.version_info[0]>=3:
                raise FileNotFoundError(self)
            else:
                raise OSError(errno.ENOENT, self)
